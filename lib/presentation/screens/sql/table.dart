...
class Table {
  ...
  bool get modified {
    ...
    return (chg.inserted.isNotEmpty ||
        chg.deleted.isNotEmpty ||
        chg.updatedNew.isNotEmpty);
  }
  ...
  Table(SqlConnection conn, String tableName, List<Map<String, dynamic>> rows,
    ...
    originalrows = _copyRows(this.rows);

    // build column definitions
    this.columns = [];
    for (var coldef in columns) {
      this.columns.add(ColumnDefinition.fromMap(coldef));
    }

    // add _originalIndex field
    _addOriginalIndexField(this.rows);
    _addOriginalIndexField(originalrows);
  ...
  Map<String, dynamic> newRow() {
    Map<String, dynamic> newRow = <String, dynamic>{};
  ...
  Future post() async {
    var postCompleter = Completer();

    // calculate changes
    ChangeSet chg = _detectChanges();

    // if no changes, does not call server
    if (chg.inserted.isEmpty && chg.deleted.isEmpty && chg.updatedNew.isEmpty)
    ...
    _conn.postBack(chg).then((response) {
      ...
      _addOriginalIndexField(rows);

      // update is ok, so accept changes
      originalrows = _copyRows(rows);
